...
df_out.to_csv(os.path.join(config['source_dir'], 'sims_params.csv'))
...
for idx, row in df_out.iterrows():
    ...
    sim_dir = os.path.join(config['source_dir'], str(idx))
...
# Create job files
job_dir = os.path.join(config['source_dir'], 'jobs')
os.makedirs(job_dir)
...
if config['move_to_supermuc']:
    ...
    subprocess.call(
        ['/bin/zsh', '-i', '-c', 'rsync -a %s $SMS/%s --delete --progress' % (
            config['source_dir'], config['remote_dir'])])
